...
import { Plus, Edit2, Trash2, TrendingUp, TrendingDown, Clock, CheckCircle, XCircle, CalendarDays } from 'lucide-react';
...
export default function ScheduledEntries() {
  const { data, addScheduledEntry, updateScheduledEntry, deleteScheduledEntry } = useApp();
  const { user } = useAuth();
  ...
  const activeCategories = tab === 'income' ? incomeCategories : expenseCategories;
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className={`border rounded-xl p-4 flex items-center gap-3 ${overdueCount > 0 ? 'bg-orange-50 border-orange-200' : 'bg-gray-50 border-gray-200'}`}>
          ...
          <div>
            ...
            {overdueCount > 0 && <p className="text-xs text-orange-500">Will apply on next page load</p>}
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        {filtered.length === 0 ? (
          <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
            ...
            <p className="text-gray-300 text-xs">Add entries to automatically create transactions on their scheduled date</p>
          </div>
        ) : (
          <div className="space-y-2">
            {filtered.map(e => {
              ...
              return (
                <div key={e.id} className={`bg-white rounded-xl border p-4 flex items-center gap-4 ${isOverdue ? 'border-orange-200 bg-orange-50/30' : 'border-gray-200'}`}>
                  {/* Type indicator */}
      ...
      <Modal
        ...
        title={editing ? `Edit Scheduled ${tab === 'income' ? 'Income' : 'Expense'}` : `Add Upcoming ${tab === 'income' ? 'Income' : 'Expense'}`}
      >
        <div className="space-y-4">
          ...
          <div className="bg-blue-50 rounded-lg p-3 text-xs text-blue-700">
            <strong>How it works:</strong> This entry will automatically become a real transaction on {form.date ? new Date(form.date + 'T00:00:00').toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }) : 'the selected date'}, and your account balance will be updated.
          </div>
